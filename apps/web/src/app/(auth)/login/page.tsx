...
const Login: FC<LoginProps> = ({}) => {
  ...
  const { mutate: login } = useMutation({
    ...
    onError: (err: AxiosError) => {
      toast.error("Forkert adgangskode eller email");
    },
    mutationKey: ["login"],
  });
  ...
  return (
    <div className="max-w-screen-lg mx-auto h-[calc(100vh-100px)] grid grid-cols-2 p-5">
      <Link
        href="/signup"
        className={cn(
          buttonVariants({
      ...
      <div className="grid place-content-center gap-5 h-full w-full p-10">
        ...
        <p className="text-2xl max-w-[350px] text-center">
          Log ind på Affaldsguiden for at anmelde stationer
        </p>
      </div>

      <div className="h-full w-full flex items-center">
        <div className="bg-white w-full p-10 rounded-3xl">
          <h1 className="text-4xl font-bold">Log ind</h1>
          <form
