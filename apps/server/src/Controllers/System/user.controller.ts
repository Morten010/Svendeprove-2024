...
class UserController {
  ...
  list = async (req: any, res: any) => {
    ...
    const qp: any = QueryParamsHandle(req, "id, firstname, lastname");
    ...
    if (firstname && lastname && email && password && org_id) {
      ...
      if(user){
        return res.status(409).send({
          message: "Bruger eksistere allerade"
        })
      }

      try {
        // Opretter record
        const model: any = await User.create(req.body);
  ...
  update = async (req: any, res: any) => {
    ...
    if (id && firstname && lastname && email && password && org_id) {
      try {
        // Opretter record
        const model: any = await User.update(req.body, {
          where: { id: id },
  ...
  update_value = async (req: any, res: any) => {
    // Destructure assignment af id.
    const { user_id, field, value } = req.body || 0;
    // Tjekker felt data
    if ((user_id as any, field as any, value)) {
      try {
        const dataObj = {};
        // @ts-ignore
        dataObj[field] = value;
